...
-- 원본의 MANAGER는 남은 실제 직위라 이름을 그대로 살린다(해당 인원 1명).
...
-- id는 계수 순이 아니다 — 매니저가 나중에 들어와 9를 차지했다(인원 행 보존).
...
-- ASSUMPTION: 원본의 EXECUTIVE(대표·부사장) 중 is_admin이 아닌 부사장은 부문장에
...
-- 인원 43명 + 시스템 계정 1명 ------------------------------------------------
-- billable=false: 경영관리팀 전체(1~4) + AX사업기획부 subtree 전체(5~10) = 10명.
--   부록 B의 "프로젝트 수행 0인 지원조직 3부문 10명" 규칙을 새 조직에 적용한
--   것이다(구 '프로텐'·'관리•마케팅부'가 경영관리팀으로 합쳐졌다).
...
INSERT INTO people
  ...
  (1,  '박재완',              2,  1, 1, 1.0, false, false, true, 0),
  (2,  '천용우',              2,  2, 2, 1.0, false, false, true, 0),
