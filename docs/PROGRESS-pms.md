...
- **다음 작업:** 2026-08-06 공용 변경 2건(완료 전이 재설계·billable)에 대한 **MCP 담당 확인** → 게이트 P 승인 → PMS-M0 스캐폴드. `list_maintenance_logs` id의 도메인 확인(프로젝트:Maintenance 1:1 vs 1:N — M-1 카탈로그 공백 답변용)과 시드 적재 잔여(월별 M/M·유지보수 데모·billable 팀 목록)는 PMS-M1 전 결정. 유저_시나리오 HTML 렌더링(md 생성 방식 — 게이트 P 리뷰 보조) 예정
