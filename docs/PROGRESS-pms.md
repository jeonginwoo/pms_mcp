...
## 현재 상태 (2026-08-25 — `?phase=` 필터: M1 pms 트랙에 A8 하나만 남았다)

- **`?phase=SALES|SOLUTION` 서버 필터 + 화면 phase 탭 완결.** §12 검수 기획 결정 4건 중 ②가 닫혔고, **M1 pms 트랙에 남은 것은 A8(프로젝트별 권한 커스텀) 하나**다. 실측: 무필터 382 · 영업 29 · 솔루션 353 · 모르는 값은 §7 봉투 400(`field: phase`)
