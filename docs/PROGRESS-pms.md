...
- **범위에서 빠진 것 — H1-4·F1-5**: 승인 범위에 있었으나 **대응이 정의되지 않아 못 했다**. H1-4의 설정 칸은 `{progress, project, org, weekly}` 넷인데 `NotificationType`은 다섯이고, `weekly`는 유형이 아니라 주기다. 어느 유형이 어느 칸인지 정한 문서가 없다 → §12 등재, 필터는 항상 통과
- 다음 작업: **H1-4 대응 결정 → F1-5** · **F2·F3 스케줄러**(상태 전이 과부하 공백도 함께) · **SSE(F1-4)**
