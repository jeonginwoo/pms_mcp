...
## 현재 상태 (2026-08-24 — 유지보수에 쓰기 입구가 생겼다)

- **EPIC D 쓰기의 절반이 섰다**: D2(계약·사이트 등록/수정) 실구현 — 라우트 4종·권한 플래그 판정·낙관적 락·감사. **남은 것은 D3**(이슈 등록·처리·코멘트)과 **D1 이관**(모듈 방향 결정 선행)이다
- **D3을 다음으로 미룬 것은 범위 판단이다**(사용자 결정): D3-1 담당자 알림이 결정 2건을 함께 끌고 온다 — `NotificationType` 확장 · §12의 "notification 루트 계약 4종의 자리". 쓰기 구현과 그 리팩터링을 한 diff에 섞지 않는다
- **유지보수 화면은 아직 조회 전용이다** — 이번에 열린 쓰기 라우트를 부르는 화면이 없다. 지금 "화면이 비어 있는" 지점은 **알림 벨**과 **유지보수 쓰기** 둘이다
- **검증**: `verify.sh pms` PASS — 테스트 **464개** · Flyway V1~V12(이번 작업은 마이그레이션 없음 — 표 모양이 그대로다)
- **다음 작업 후보**:
  1. **D3 (D-b 나머지)** — 이슈 등록·처리·append-only 코멘트 + 담당자 알림. 결정 2건이 선행
  2. **유지보수 쓰기 화면** — 방금 생긴 라우트 4종. 작고 눈에 보인다
  3. **F2·F3 스케줄러** · **F1-4 SSE** · **project 잔여 AC(A6-3 → A8)** — 아래 이전 상태의 판단 그대로
