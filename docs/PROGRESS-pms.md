...
- **B2-1 후속 결정 ③④ pms 측 결정 완료**(2026-08-11 — 공용 결정 기록 2행, **MCP 담당 확인 대기**): ③ `get_utilization` scope에 `COMPANY` 추가 + 응답에 team·division · ④ `search_maintenance` 신설(카탈로그 7종→8종, keyword = 계약명·계약사·**사이트명**). PMS 측 반영 = **PRD-pms v2.7**(C1-6 신설·D4-1 확장·§7·12장)
- **다음 작업:** ③④ 양측 합의 성립 대기 — 합의 후 host 트랙이 카탈로그·description·eval 반영. **합의 전까지 C1-6·D4-1 keyword는 구현하지 않는다**(12장). PMS-M0 스캐폴드는 게이트 M-1 통과 후
- **차단 요소:** 없음

## 이전 상태 (2026-08-10)

- **가동률 의미 재정의 결정 + MCP 확인 완료**(2026-08-10 동석 — 공용 결정 기록): 배정 M/M=실투입 계획·오버부킹=기본>100·보정=×coeff 단가 가중·PM 하드 제외 미채택. 상위 PRD §3·**PRD-pms v2.6** 반영 완료. **시드 적재 정책 전량 해소** — 월별 M/M 규칙·billable 목록·유지보수 시트 변환(`reference/seed/maintenance.json`) 모두 확정(부록 B)
- **유저_시나리오 §10 정비 완료**(2026-08-10 — v1.2): WS-07 신설(유지보수 직접 등록·이슈 운영·전사 조회 — 2026-08-06 미해결분 해소) + 게이트 P 결정 ⑦·가동률 재정의·시드 확정치 정합화. pms 재량 범위(§10)만 수정 — 대기 중 후보 소진
- **다음 작업:** pms 트랙은 게이트 M-1 대기(M-1 목업 MCP 서버는 host 트랙 진행). **PMS-M0 스캐폴드는 게이트 M-1 통과 후.** 대기 중 후보 없음
- **차단 요소:** 없음
