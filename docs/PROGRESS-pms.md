...
- **다음 작업:** **화면 프로토타입 리뷰 마무리**(두 담당 — `prototype/`, 피드백 3라운드 반영 완료·디자인 시안 G 적용) → **기획 결정 후보 8건 확정**(2026-08-09 세션 로그 — 특히 ④시스템 ADMIN 계정·⑦권한 그룹 일반화는 상위 PRD §4 공용 변경이라 **MCP 담당 합의 필수**, ⑧조직 트리는 identity 모델 변경) → PRD 반영 → **게이트 P 승인**(사람 — PRD-pms v2.4 포함 일괄) → PMS-M0 스캐폴드. 시드 적재 잔여(월별 M/M·billable 팀 목록·유지보수 시트→JSON 변환+engineerId 매핑·OFFSITE→원격 변환)는 PMS-M1 전 결정
