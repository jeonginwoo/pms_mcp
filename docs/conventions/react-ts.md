...
> Scope: the chat widget and the PMS front-end (`frontend/`, pms track).
> Referenced from `pms/CLAUDE.md`. The existing prototype (`frontend/`) is
> written in JS — **new files are written in TS**; migrate existing files
> incrementally as you touch them.
...
## 3. State & data
- Local state via `useState`/`useReducer`; server state goes through the fetch wrapper (`src/api.js`) — no direct `fetch` calls inside components.
- Global state follows the existing store pattern (`store.jsx`); introducing a new library follows the `기술_선택_근거` process.
- Never omit the `useEffect` dependency array. Do not suppress lint warnings.

## 4. Chat-widget-specific rules (PRD FR-AI series — re-confirm numbers against PRD v2.0)
