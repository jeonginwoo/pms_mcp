...
- **Inter-module communication**: only through the types the other module put **in its root package** — that is its whole public API (see the `package-info.java` rule above; `@NamedInterface` is not used). Every sub-package — `controller/`, `service/`, `service/impl/`, `service/dto/`, `service/entity/`, `repository/` — is module-internal, so **entities and repositories cannot cross a module boundary** and links are by id (PRD-pms §0). A module whose root is empty exports nothing: to let another module call it, move the contract (and the DTOs it returns) to the root — a visible act, not a config flag. Verified by `ModularityTest`.
