...
- **G1의 잔여 임계경로는 EPIC C 하나다**(2026-08-23 재갱신): 도구 8종 중 **6종 실연결**(person 2 + maintenance 2 + project 2). 남은 2종은 가동률뿐이고 **EPIC C 실구현**이 없어 계약만으로는 답을 만들 수 없다 — 어댑터가 할 일은 없다. ~~G1 선행으로 남은 host 몫은 eval 코퍼스 시드 재앵커 하나다~~ → **재앵커 2026-08-23 완료**로 host 몫의 G1 **선행**은 끝났고, 남은 host 작업(실서버 전환·eval 자동 실행 스크립트)은 게이트 **준비**분이다.
...
- [ ] **resource 포트 실연결** (`get_utilization`·`list_overbooked`) — **선행이 하나만 남았다: `resource` 루트 승격 소유자 합의**. ~~EPIC C 실구현~~(2026-08-23 완료, PR #29)·~~조직 id 후속~~(`WorkforceProfile.teamOrgUnitId`·`divisionOrgUnitId` 도착 — 2026-08-24 확인)은 해소됐고, 남은 것은 `resource` 모듈 루트가 비어 있다는 것이다. git-workflow §3대로 **파일을 만들기 전에 소유자를 결정 기록으로 정한다**(MCP 담당 제안 = 선례 3건과 같이 PMS 담당. 2026-08-24 결정 기록 ⑤). 어댑터가 쓸 면은 좁다 — (월·화자·scope) → 인원별 행 + 과부하 목록. `UtilizationView` 7필드가 `UtilizationEntry`를 채우기에 충분함은 실측 확인. 합의되면 이 항목이 마지막 도구 2종을 열어 **8종 전량 실연결**이 된다
