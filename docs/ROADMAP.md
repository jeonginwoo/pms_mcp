...
- [ ] **EPIC F 알림 + SSE + 스케줄러** (F1~F3) — **1차 완료 2026-08-24**: 이벤트 배관 신설(이 프로젝트 첫 도메인 이벤트) + F1-1 과부하 알림 · F1-2 멱등 · F1-3 목록·읽음 · F3-3 회수. notification 501 **4 → 0**. 착수 전 §3↔§8 순환 모순을 풀었고(이벤트 한 방향 확정 — 결정 기록), "팀장"은 이름·scope가 아니라 **"프로젝트 생성" 플래그**로 판정한다(사용자 결정). **남은 것**: F1-5(H1-4 설정 단위가 `NotificationType`과 맞지 않아 대응 결정 선행 — §12) · **F2·F3 스케줄러** · **F1-4 SSE**(토큰 마스킹과 한 묶음). 파생 공백: **상태 전이로도 과부하가 생기는데 그 이벤트가 §8에 없다**(계약대기→진행중이면 배정이 분자에 들어온다)
