...
- [ ] 목업 MCP 서버 (구현 노트 목업 전략, stateless 지향으로 검증) — **스캐폴드 + B2-0 배관 검증 완료**(2026-08-10, `pms-mcp-mock/` — 도구 7종·테스트 32개·오염 레코드 기심음). 잔여: B2-1 자연어 검증(사용자 동석) · B2-2 JWT 권한 흐름 · B2-3 자체 호스트 연결
- [ ] `get_project` 분리 여부 결정 (실험 데이터 기반 — 2026-07-31 유예 결정)
- [ ] `search_projects` 상세 응답 `myRole` 포함 여부 결정 (실험 데이터 기반 — 2026-08-03 유예 결정)
- [ ] 카탈로그 공백 2건 해소 — `get_utilization` 전사/타부문 scope · `list_maintenance_logs` **id 확보 경로**(id 의미는 2026-08-06 해소 — 계약/이슈 id, projectId 단순화 불가: 유지보수 재설계 결정 기록) (유저_시나리오 §7 발견, 카탈로그 변경이면 결정 기록 경유)
- [ ] eval 입력·기대값 확정
