...
| EPIC | 상태 | 남은 것 |
|------|------|---------|
| **A 프로젝트** | 대부분 실구현 — A1 생성 · A2 진척률 2단계 · A3 조회·가시성 · A4 삭제 · A5 수정·상태전이 · A6-1·2·4·5 PM 교체 · **A6-3·A6-6·A6-7 역할 지정**(2026-08-24 — `PUT /projects/{id}/roles`, 화면은 배정 목록의 드롭다운+확인) · A7 완료·재개 · **`?phase=` 목록 필터 + phase 탭**(2026-08-25 — `ProjectPhase`가 status 집합을 들고 양방향으로 읽힌다: 단건은 status→phase, 목록 필터는 phase→status 집합. 목록 응답에도 파생 `phase`를 실어 화면이 표를 갖지 않게 했다) | **A8 전체**(`GET`/`PUT /permissions` — A8-5·A8-6이 진척률·배정 판정에 침투) |
