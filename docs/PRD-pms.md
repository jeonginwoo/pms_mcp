...
- **인력 목록의 권한 그룹 컬럼 (2026-08-24 등재)**: `PersonSummary`에 `groupId`는 있고 **그룹 이름이 없다**. 관리 권한자는 이미 그룹 목록을 받아 두므로 화면 매핑으로 되지만, 비관리자에게 보이려면 DTO에 `groupName`을 실어야 한다 — 권한 정보를 어디까지 노출할지가 먼저다(`whoami`는 FR-AI-16에 따라 플래그를 담지 않는다는 선례가 있다)
