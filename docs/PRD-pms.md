...
| **F 알림** | **F1 전량 실구현**(2026-08-24) — F1-1 과부하 알림(이벤트 배관 신설) · F1-2 멱등 · F1-3 목록·읽음 · **F1-5 설정 필터** · F3-3 회수. 도메인 이벤트가 이 프로젝트에서 처음 돈다. **유형이 6종이 됐다**(2026-08-24 D3 — `ISSUE_ASSIGNED` "이슈 담당 지정". 이름이 "등록"이 아닌 것은 수신자가 담당자이기 때문이다 — 사용자 결정) | **F1-4 SSE 미착수**(토큰 마스킹과 한 묶음 — 화면은 그동안 부팅+벨 열기 재조회로 돈다) · **F2·F3 스케줄러 미착수** |
...
- **`ProjectCompleted`·`ProjectReopened`가 한 번도 발행되지 않는다 (2026-08-25 실측 — 미해결)**: §8이 둘을 명세했고 `NotificationType.PROJECT_COMPLETED`가 존재하는데 **발행 지점이 0곳**이다. 이어서 **F3-3 회수(`NotificationService.withdrawUnread`)는 실사용 호출자가 0건**이다 — 테스트만 직접 부른다. §10 F행과 2026-08-24 세션 기록이 "F3-3 회수"를 완료로 적고 있는데 그것은 **능력이고 배선이 아니다**(재개해도 완료 지연 알림이 회수되지 않는다). 이관 알림은 이 공백과 무관하게 돈다(발행자가 maintenance다). **F2·F3 스케줄러와 함께 볼 일**이다 — 그때 `ProjectCompleted` 발행과 `ProjectReopened` → 회수 배선을 같이 세우는 것이 맞다
