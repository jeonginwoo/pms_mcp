...
| EPIC | 상태 | 남은 것 |
|------|------|---------|
| **A 프로젝트** | 대부분 실구현 — A1 생성 · A2 진척률 2단계 · A3 조회·가시성 · A4 삭제 · A5 수정·상태전이 · A6-1·2·4·5 PM 교체 · **A6-3·A6-6·A6-7 역할 지정**(2026-08-24 — `PUT /projects/{id}/roles`, 화면은 배정 목록의 드롭다운+확인) · A7 완료·재개 | **A8 전체**(`GET`/`PUT /permissions` — A8-5·A8-6이 진척률·배정 판정에 침투) · `?phase=` 목록 필터 |
...
| **F 알림** | **F1·F2·F3 실구현**(2026-08-25 — F1-4 SSE만 남았다). F1-1 과부하 · F1-2 멱등 · F1-3 목록·읽음 · F1-5 설정 필터 · **F2-1·F2-2 마감 임박**(종료일 D-7 진행중 → PM, 일일) · **F3-1·F3-2 완료 지연**(100%인 채 7일 → PM·PL) · **F3-3 회수 배선**. 유형 6종(`ISSUE_ASSIGNED` 포함). **스케줄러는 project에 두고 이벤트를 발행한다**(2026-08-25 — "어느 프로젝트가 D-7인가"는 프로젝트 판단이고, notification 밖에서 `notify`를 부르지 않는 규칙에 스케줄러도 예외를 두지 않는다). 100% 도달 시각은 **Flyway V15** 컬럼(F3-1이 허용한 단순 구현 — 시드 적재는 시각을 남기지 않는다: 적재 시점은 도달 시점이 아니다) | **F1-4 SSE 미착수**(`?access_token=` 인증과 로그 마스킹이 한 묶음 — 화면은 그동안 부팅+벨 열기 재조회로 돈다) |
