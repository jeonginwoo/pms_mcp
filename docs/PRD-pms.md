...
| **D 유지보수** | **조회 실구현**(2026-08-23) — 모듈·엔티티 4종·Flyway V9·시드 적재(계약 105·사이트 157·연락처·이슈 14) · D4-1 목록(keyword 3종 매칭) · D4-2 상세 · D4-3 전사 공개 · **D3-4 이슈 조회**(미배정 필터 포함) · `IssueComment` 표·조회(적재 0건 — 시드에 본문 없음). **쓰기 D2 실구현**(2026-08-24) — D2-1 계약 직접 등록(`max(id)+1` · sourceProjectId 없음) · D2-2 수정(낙관적 락 · **삭제 API 없음**, 종료는 상태로) · D2-3 "계약 관리" 플래그 403 · D2-4 사이트 등록·수정(연락처 동봉, PUT은 전체 교체). 감사는 `projectId=null`이라 통합 로그(G1-3)에만 나온다. 부수: `SiteView`에 `version` 신설(§7 "단건 응답은 version을 포함한다" — 없으면 화면이 낙관적 락에 넣을 값을 못 얻는다) | **D1 이관**(§5에 완료→유지보수중 전이가 없어 `Project`에 전용 메서드 신설 필요 + 모듈 방향 결정) · **D3-1~D3-3 이슈 등록·처리·코멘트**(D3-1 담당자 알림이 `NotificationType` 확장과 §12 "notification 루트 계약 4종의 자리"를 함께 끌고 온다 — 그래서 D2와 분리했다) · **화면**(쓰기 라우트를 부르는 화면이 아직 없다) |
...
| **F 알림** | **F1 전량 실구현**(2026-08-24) — F1-1 과부하 알림(이벤트 배관 신설) · F1-2 멱등 · F1-3 목록·읽음 · **F1-5 설정 필터** · F3-3 회수. 도메인 이벤트가 이 프로젝트에서 처음 돈다 | **F1-4 SSE 미착수**(토큰 마스킹과 한 묶음 — 화면은 그동안 부팅+벨 열기 재조회로 돈다) · **F2·F3 스케줄러 미착수** |
...
| **H 내 계정** | 부분 — H1-1 `GET /api/me` · **H1-4 notif-prefs**(2026-08-24 — 설정 단위를 `NotificationType` 5종으로 확정. 화면은 알림 벨의 설정 모달) | `GET /api/me/account` · H1-2 profile · H1-3 password |
