...
- **노드별 프로젝트 수가 `OrgUnitView`에 없다 (2026-08-24 등재)**: 부록 A가 요구하는 항목이고 프로토타입 화면이 이미 보여 준다(그 정의 = **그 노드가 PM 소속 노드인, 삭제되지 않은 프로젝트 수** — 직속 기준이고 subtree 합계가 아니다. 프로토타입 실측으로 `projects.json`의 `team` 카운트와 11개 노드 전부 일치했다). person이 project를 알아야 해서 **순환**이라 `AssignmentCountPort` 선례대로 person이 포트를 정의하고 project가 구현해야 한다
