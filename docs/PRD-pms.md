...
| **E 조직·사용자** | **전량 실구현**(2026-08-24) — E1-1 소속 이동(+E1-2 경고) · E2-1 등록 · E2-2 수정 · E2-3 비활성 · E2-4 플래그 403 · E2-5 시스템 계정 422 · E3-1 생성 · E3-2 개명 · E3-3 `IN_USE` 삭제 거절 · E3-4 subtree 가시성 · **E4 직급 3종** · **E5 권한 그룹 3종** | 없다. 부수: 직급·권한 그룹 id를 **시퀀스**로 발급(Flyway V11) — 하드 삭제가 있는 명시 id 참조 데이터의 규칙(부록 B)을 따랐다 |
| **F 알림** | **F1 전량 실구현**(2026-08-24) — F1-1 과부하 알림(이벤트 배관 신설) · F1-2 멱등 · F1-3 목록·읽음 · **F1-5 설정 필터** · F3-3 회수. 도메인 이벤트가 이 프로젝트에서 처음 돈다 | **F1-4 SSE 미착수**(토큰 마스킹과 한 묶음) · **F2·F3 스케줄러 미착수** |
...
| **H 내 계정** | 부분 — H1-1 `GET /api/me` · **H1-4 notif-prefs**(2026-08-24 — 설정 단위를 `NotificationType` 5종으로 확정) | `GET /api/me/account` · H1-2 profile · H1-3 password |
