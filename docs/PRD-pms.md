...
| **D 유지보수** | **조회 실구현**(2026-08-23) — 모듈·엔티티 4종·Flyway V9·시드 적재(계약 105·사이트 157·연락처·이슈 14) · D4-1 목록(keyword 3종 매칭) · D4-2 상세 · D4-3 전사 공개 · **D3-4 이슈 조회**(미배정 필터 포함). **쓰기 D2 실구현**(2026-08-24) — D2-1 계약 직접 등록(`max(id)+1` · sourceProjectId 없음) · D2-2 수정(낙관적 락 · **삭제 API 없음**, 종료는 상태로) · D2-3 "계약 관리" 플래그 403 · D2-4 사이트 등록·수정(연락처 동봉, PUT은 전체 교체). **쓰기 D3 실구현**(2026-08-24) — D3-1 이슈 등록(`max(id)+1` · 담당 기본값 = 사이트 engineerId · `MaintenanceIssueRegistered` 발행 → 담당자 알림) · D3-2 상태 전이(전이 그래프는 `IssueStatus.canTransitionTo`, 흐름 밖은 409 `INVALID_TRANSITION`, 완료일은 상태가 정하고 재개하면 지운다)·담당 재배정(PATCH 의미론 — 빈 칸은 "그대로") · D3-3 append-only 코멘트. **이슈 쓰기에는 관문이 없다** — US-D3은 로그인 사용자 전체라 `ContractWriteGuard`를 타지 않는다(계약 쓰기와 판정 축이 다르다). 감사는 `projectId=null`이라 통합 로그(G1-3)에만 나오고, **코멘트는 감사 행을 남기지 않는다**(자기가 이미 불변 기록이다). 부수: `SiteView.version` 신설(D2) · `IssueView.statusCode` 신설(D3 — 처리 화면이 다음 상태를 계산하는 값. `ContractDetail.statusCode` 선례) | **D1 이관**(§5에 완료→유지보수중 전이가 없어 `Project`에 전용 메서드 신설 필요 + 모듈 방향 결정) · **화면은 전부 붙었다**(2026-08-24 — 계약·사이트 쓰기 + 이슈 등록·처리·코멘트). 파생 공백: **재배정은 알리지 않는다**(§8에 그 이벤트가 없다 — 등재) |
