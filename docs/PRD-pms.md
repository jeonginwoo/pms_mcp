...
- **"전체" 탭의 구별력을 실 데이터로는 볼 수 없다 (2026-08-25 등재 — 실측)**: §5는 유지보수중이 어느 phase에도 들지 않는다고 정하고, 그래서 "전체"가 그것을 볼 수 있는 유일한 탭이다. 그런데 시드 382건의 상태 분포가 완료 319 · 진행중 34 · 수주확정 19 · 계약대기 10이고 **유지보수중이 0건**이라, 영업 29 + 솔루션 353이 정확히 382가 된다 — 화면에서 "전체"와 두 탭의 합이 구별되지 않는다. 동작은 통합 테스트가 픽스처로 잠갔지만(이관한 프로젝트가 두 탭에서 함께 빠지는 것) **화면 시연에는 이관을 한 번 일으켜야 한다**(부록 B 시연 앵커 = 명화공업). 시드가 이관을 재현하지 않는 것이 원인이고, 그것 자체는 결함이 아니다 — 이관은 §5 전이 밖의 전용 경로다
