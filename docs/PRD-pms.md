...
| **F 알림** | **적재·조회·읽음 실구현**(2026-08-24) — F1-1 과부하 알림(이벤트 배관 신설) · F1-2 멱등 · F1-3 목록·읽음 · F3-3 회수. 도메인 이벤트가 이 프로젝트에서 처음 돈다 | **F1-4 SSE 미착수**(토큰 마스킹과 한 묶음) · **F1-5 설정 필터** — H1-4 대응이 정해져야 한다(§12) · **F2·F3 스케줄러 미착수** |
