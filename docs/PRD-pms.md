...
| 버전 / 상태 | v2.31 · **확정** (게이트 P 통과 2026-08-09. v2.6 = 가동률 의미 재정의 + 시드 정책 완결(2026-08-10 — MCP 담당 확인 완료). v2.7 = M-1 카탈로그 공백 2건 해소의 PMS 측 반영(2026-08-11 결정 기록, 2026-08-12 MCP 담당 확인 완료 — 양측 합의 성립·host 반영 완료). 이슈→계약 링크 기준 = 사이트명 포함 확정(2026-08-14 — 부록 B·결정 기록). v2.8 = 재구축·골격 확장 이후의 문서↔코드 정합(2026-08-23 — 결정 기록). v2.9 = 모듈 간 계약 2종 신설(가동률 분자·분모) + B2-1 종료일 규칙(2026-08-23 — MCP 담당 확인 요청). v2.10 = 프로젝트 시드 적재 + 인원 정본 확정(`seed_org_proten.sql`). v2.11 = 감사 조회 2뷰 실구현(G1-3·G2-2). v2.12 = maintenance 모듈 신설(EPIC D 조회분) + 시드↔모델 괴리 결정 7건. v2.13 = 도메인 루트 계약 3종 승격(안 ② 이행) + 조직 id·시드 원본 id(2026-08-23 결정 기록). v2.14 = `resource` 루트 계약 승격 + 승격 소유자 결정 선행(2026-08-24). v2.15 = 부록 B 재앵커 + 이슈 작성자 교정(2026-08-24). v2.16 = EPIC E 쓰기 5종 + §4 id 발급 규칙 승격(2026-08-24). v2.17 = 알림 이벤트 방향 확정(§3·§8) + EPIC F F1 전량 + H1-4 설정 단위 확정(Flyway V12) — 2026-08-24. **v2.18 = 사용자 검수 반영 — US-E3에 노드 이동 AC 신설(E3-5·E3-6) · A6-3 역할 지정 실구현 표기 · 조직 재편(노드 17→18)과 직급 2종 개명(Flyway V13·V14) · 부록 A 조직/배정 화면 문구 · §12에 미구현 기획 결정 4건 등재 — 2026-08-24, 공용 결정 기록 2행. **v2.19 = EPIC D 쓰기 D3 실구현(이슈 등록·처리·append-only 코멘트) + `NotificationType` 6종화(`ISSUE_ASSIGNED`) + §12 notification 루트 계약 판정 종결·재배정 알림 공백 등재 — 2026-08-24. **v2.20 = EPIC D 완결(D1 이관 — 역포트 `project.HandoverPort`, 한 트랜잭션, D1-2 문면 `INVALID_TRANSITION`으로 정정) + §12에 `ProjectCompleted` 미발행 등재 — 2026-08-25. **v2.21 = EPIC F 스케줄러(F2·F3) + §8에 `ProjectReminderDue` 2종 신설 + `ProjectCompleted`·`ProjectReopened` 배선(F3-3 회수가 실제로 돈다) + Flyway V15 — 2026-08-25. **v2.22 = F1-4 SSE로 EPIC F 완결 — 쿼리 토큰은 스트림 라우트 하나로 좁히고 Nginx 마스킹을 배포 요구로 등재 — 2026-08-25. **v2.23 = EPIC H 완결(`/api/me/account`·H1-2·H1-3) + `/api/me/*`는 데이터를 가진 모듈이 갖는다는 규칙 명시화 — 2026-08-25. **v2.24 = `?phase=` 목록 필터 + phase 탭 실구현(§12 검수 결정 ② 종결) — phase 표를 `ProjectPhase` 한 벌에 두고 양방향으로 읽는다 · 목록 응답에 파생 `phase` 동봉 · §12에 "전체 탭의 구별력을 실 데이터로 볼 수 없다" 등재 — 2026-08-25. **v2.25 = phase 화면 배치 개정(사용자 결정) — 한 목록의 단계 탭에서 내비 항목 둘(영업·프로젝트)로. "전체" 화면은 없어지고 §5의 "유지보수중은 계약이 담당한다"가 화면에서도 성립한다 — 2026-08-25. **v2.26 = US-A8 권한 커스텀 실구현(Flyway V16) — EPIC A가 닫혔다. 표는 한 벌·병합은 한 곳이고 판정 침투는 호출부 무변경 — 2026-08-26. **v2.27 = notification 루트 계약 5종을 service/로 이동(§12 실행) — 그 모듈의 루트가 0이 됐다. 순수 리팩터·동작 무변경 — 2026-08-26. **v2.28 = §12 person 공백 2건 해소 — E2-2가 `billable`을 받고(부록 B의 "운영 중 개인 단위 수정 가능" · 감사 스냅샷 포함), 인력 목록에 관리자 전용 권한 그룹 열(서버 DTO는 넓히지 않고 화면 매핑 — 경계는 그 라우트의 403이다) — 2026-08-26. **v2.29 = 노드별 프로젝트 수(`OrgUnitView.projectCount` · `ProjectCountPort`) + E3-3 문면 전량 성립 — 2026-08-26. **v2.30 = §12 검수 결정 ③ 해소(E2-3a·b·c — 퇴사가 참여자 배정을 끊고 PM은 409로 거절, `AssignmentReleasePort` 신설) + **선행 결함 정정**("진행 중 배정"이 배정 상태만 봐 완료 건 384/462를 세고 있었다 — 정의를 `ProjectStatus.LIVE` 한 벌로) + §10 횡단 표 stale 2행 실측 정정 — 2026-08-26**. **v2.31 = 이슈 정정·삭제·코멘트 수정(AC D3-5·D3-6·D3-7 신설 · D3-3 append-only 폐기 · Flyway V17) — 관문이 행위별로 갈린다. 착수 계기는 "제목 오타를 고칠 방법이 없었다" — 2026-08-26** |
...
### v2.19 반영 (2026-08-24 — EPIC D 쓰기 완결분(D3) + 알림 유형 6종, PROGRESS 결정 기록)

- **§10 EPIC D**: D3-1~D3-3 실구현 반영. 이슈 쓰기에는 **관문이 없다**(US-D3 = 로그인 사용자
  전체) — 계약 쓰기의 "계약 관리" 플래그와 판정 축이 다르다는 것이 코드에도 드러났다
  (`ContractWriteGuard`를 타지 않는 첫 쓰기 계약).
- **§10 EPIC F·H**: `NotificationType`이 6종이 됐다 — `ISSUE_ASSIGNED`("이슈 담당 지정",
  사용자 결정). H1-4 설정 화면은 유형 전체를 서버 응답에서 그리므로 칸이 자동으로 늘었다.
- **§12 해소 1건**: "notification 루트 계약 4종의 자리" — D-b가 마지막 반증 기회였고 반증되지
  않아 `service/`로 내리는 판정이 확정됐다(이동은 별 단위).
- **§12 신규 1건**: 이슈 **재배정에는 알림이 없다**(§8에 그 이벤트가 없다).
- 부수: `IssueView.statusCode` 신설 — 처리 화면이 다음 상태를 계산하는 값이고
  `ContractDetail.statusCode`(v2.9 부수)와 같은 근거다.

### v2.20 반영 (2026-08-25 — EPIC D 완결(D1 이관) + 모듈 방향 역포트, PROGRESS 결정 기록)

- **§6 D1-2 문면 정정**: `409 NOT_COMPLETED` → **`409 INVALID_TRANSITION`**(사용자 결정).
  같은 상태 기계의 다른 칸(A7)이 이미 그 코드를 쓰고, `ErrorCode`를 늘리면 `mcp` 모듈의
  예외 매핑이 함께 움직여 소유 경계를 넘는다.
- **§10 EPIC D 전량 실구현** — D1 이관이 마지막 칸이었다. 계약·사이트 생성과 상태 전이가
  한 트랜잭션이고, 그 원자성을 통합 테스트가 **실물 행 수**로 실증한다.
- **§3 모듈 의존 1건 신설**: **maintenance → project**(역포트 `project.HandoverPort`).
  project와 maintenance는 실측상 서로를 모르는 형제였고, 잠재 의존이 이미
  maintenance → project를 가리켰다(maintenance가 `sourceProjectId`를 저장하고 D4-2가
  원 프로젝트 링크를 요구한다) — 반대로 그었다면 그때 순환이 된다.
- **§12 신규 2건**: 이관 알림 수신자를 구현이 정했다(§8 미명세) ·
  **`ProjectCompleted`·`ProjectReopened`가 한 번도 발행되지 않고 F3-3 회수는 배선이 없다**.

### v2.21 반영 (2026-08-25 — EPIC F 스케줄러(F2·F3) + §8 이벤트 2종 신설, PROGRESS 결정 기록)

- **§8에 `ProjectReminderDue` 2종 신설**(project → notification). §8은 F2·F3의 알림 *유형*만
  정하고 만들어지는 경로를 비워 뒀는데, "notification 밖에서 `notify`를 부르는 모듈은 없다"는
  규칙(2026-08-24 확정)에 스케줄러도 예외를 둘 수 없다 — 그래서 스케줄러도 발행한다.
- **§8 `ProjectCompleted`·`ProjectReopened` 배선**: 명세만 있고 발행 지점이 0곳이던 둘을
  `ProjectLifecycleChanged`로 채웠다. **F3-3 회수가 이로써 실제로 돈다.**
- **§10 EPIC F**: F1·F2·F3 실구현. 남은 것은 F1-4 SSE 하나다.
- **스키마 V15**: `projects.hundred_reached_at` — F3-1이 재는 기준점. AC가 허용한 단순 구현이고,
  **시드 적재는 시각을 남기지 않는다**(적재 시점은 도달 시점이 아니다 — 7일 뒤 시드 전체가
  알림을 쏘는 것을 막는다).
- **§12 해소 1건**: `ProjectCompleted` 미발행(같은 날 등재·해소).

### v2.22 반영 (2026-08-25 — F1-4 SSE로 EPIC F 완결, PROGRESS 결정 기록)

- **§10 EPIC F 전량 실구현** — SSE가 마지막 칸이었다.
- **§7 SSE 인증의 범위를 좁혔다**: `?access_token=`은 **스트림 라우트 하나만** 받는다.
  common의 호출자 리졸버를 넓히면 모든 라우트가 쿼리 토큰으로 불릴 수 있게 되고,
  토큰이 액세스 로그에 남는 자리가 앱 전체가 된다.
- **배포 요구 등재**: Nginx 로그 포맷의 `access_token` 마스킹은 **앱 밖의 몫**이다
  (구현 노트 §6). 앱은 그 값을 스스로 로그에 남기지 않는 것까지 책임진다 —
  예외 문구에도 토큰이 들어가지 않는다.
- 부수: **재연결 복구는 목록 재조회다** — 서버 재생(`Last-Event-ID`)은 같은 세션의
  리뷰에서 걷어냈다(재생분이 그 사람의 **모든 연결**로 브로드캐스트돼 다른 탭의 커서를
  되감고 · 상한에 걸려 잘린 것을 알릴 방법이 없고 · 재생 중 예외가 이미터를 누수시켰다).
  AC F1-4가 요구하는 것은 *"재연결·재조회 시 반영"*이고 **화면이 연결될 때마다 목록을
  재조회하는 것이 그 문면 그대로**다. 재연결은 앱이 직접 한다(브라우저에 맡기면 URL에
  굳은 낡은 토큰으로 붙어 영구 실패한다).
...
### v2.31 반영 (2026-08-26 — 이슈 정정·삭제·코멘트 수정, PROGRESS 결정 기록)

- **AC D3-5·D3-6·D3-7 신설 + D3-3 append-only 폐기**(사용자 결정). 착수 계기는 실측된
  공백이다: 등록 경로는 있는데 `PATCH`가 `status`·`assigneeId` **두 칸만** 받아
  **제목 오타를 고칠 방법이 아예 없었다**.
- **관문이 행위별로 갈린다** — 등록·처리·코멘트 추가는 US-D3대로 로그인 사용자
  전체이고, **정정·삭제**는 `IssueWriteGuard`(등록자·담당자·"계약 관리" 플래그),
  **코멘트 수정·삭제**는 작성자 본인만이다. 이슈 게시판이 전사 공개라(D4-3) 관문이
  없으면 누구나 남의 이슈를 지운다. 라우트를 쪼개지 않고 **요청 내용으로** 관문을
  가르는 이유는 "상태와 제목을 한 번에" 요청이 두 왕복·락 두 번이 되기 때문이다.
- **Flyway V17** — 본문(`content`) · 등록자(`reporter_id`) · soft 삭제(`deleted`,
  프로젝트 A4 선례) · 코멘트 `updated_at`. 앞의 셋이 nullable인 것은 시드 267건이
  본문도 작성자도 없이 적재됐기 때문이고, 그래서 **시드 이슈의 정정은 담당자나
  "계약 관리" 권한자가 든다**.
- **어댑터가 `AssignmentService.close` 대신 직접 종료하던 것과 같은 판단이 여기도
  있다**: 이슈 정정이 `ContractWriteGuard`를 재사용하지 않는다 — 그쪽은 플래그
  **하나**를 요구하는 관문이고 여기서 플래그는 **세 갈래 중 하나**다.
- **수정 흔적은 남긴다**(`updatedAt`) — 사용자 결정은 수정 허용이고 흔적 삭제가
  아니었다. tombstone(코멘트 삭제 표시)은 미채택이라 코멘트 삭제는 행을 지운다.
- **§12에 MCP 확인 요청 1건 등재**: 본문이 `list_maintenance_logs`에 실리지 않아
  웹과 챗이 다른 것을 본다. 카탈로그 결정이라 MCP 담당 소유이고 G1을 막지 않는다.

### v2.30 반영 (2026-08-26 — 퇴사 처리가 배정까지 든다 + "진행 중"의 정의 정정, PROGRESS 결정 기록)

- **§12 검수 결정 ③ 해소** — AC **E2-3a·E2-3b·E2-3c** 신설. 퇴사(비활성)가 진행 중
  **참여자** 배정을 함께 종료하고 무엇이 끊겼는지 응답 `data`에 싣는다(E1-2
  `OrgUnitMoveResult` 선례 — 막지 않으니 오류로 낼 수 없고, 알림으로 보내면 지금
  화면에서 정리하는 사람이 못 본다). **PM으로 물려 있으면 409 IN_USE**이고 아무것도
  바뀌지 않는다 — PM 공석은 A6-5가 금지하고 교체 대상은 시스템이 고를 수 없다.
- **계약은 새 포트로 나눴다**(`person.AssignmentReleasePort` — person 정의 · project
  구현). 등재문은 `AssignmentCountPort`를 넓히라고 적었지만 그 javadoc이 자기 면을
  좁힌 근거를 이미 갖고 있었다 — **질문마다 포트를 나눈다**(`ProjectCountPort` 선례,
  2026-08-26 사용자 결정). 어댑터가 `AssignmentService.close`를 재사용하지 않는 이유는
  D1 이관과 같다: 그 관문(프로젝트별 `ASSIGN`=PM)을 지나면 관리자가 남의 프로젝트
  PM이 아니라는 이유로 퇴사 처리를 못 한다.
- **착수 전 실측이 선행 결함을 드러냈다 — 이번 개정의 본론은 이쪽이다**: "진행 중
  배정"이 **배정 상태만** 보고 있었다. 완료 프로젝트의 배정이 살아 있어(ACTIVE 462건
  중 **384건**이 완료 건) E1-2 이동 경고가 한 사람에게 **"진행 중인 배정 128건"**이라
  답했고 실제로 물려 있는 것은 **5건**이었다. 근거로 적혀 있던 *"완료 시 배정이 이미
  종료 처리된다(AC B2-1)"*는 **사실이 아니다** — B2-1은 명시적 수동 종료다.
  정의를 `ProjectStatus.LIVE` **한 벌**로 모아 이동 경고와 퇴사 처리가 같이 읽게 했고
  (데이터는 고치지 않는다 — 2026-08-26 사용자 결정), 가동률은 기간 겹침으로 세므로
  영향이 없음을 실측으로 확인했다.
- **§10 횡단 표의 stale 2행 정정**(실측): 시드는 **전량 적재**되고(382 프로젝트 · 105
  계약) `/mcp` 도구는 **8종 전량 실연결**이다. 둘 다 v2.10~v2.12·2026-08-24에 이미
  참이 됐는데 이 표만 따라오지 않았다.

### v2.29 반영 (2026-08-26 — 노드별 프로젝트 수 + E3-3 범위 확정, PROGRESS 결정 기록)

- **§12 "노드별 프로젝트 수" 해소** — `OrgUnitView.projectCount` 신설. 정의는 등재문
  그대로 **그 노드가 PM 소속 노드인, 삭제되지 않은 프로젝트 수**(직속 · subtree 합계
  아님)이고, 계약은 **`person.ProjectCountPort`**(person 정의 · project 구현)다.
  키가 조직 노드가 아니라 **PM**인 것이 이 계약의 요점 — "누가 어느 노드에 속하는가"는
  person의 지식이라, 노드별로 접어 달라고 하면 같은 규칙이 project에도 생긴다.
- **§6 E3-3의 문면이 처음으로 전부 성립한다**(사용자 결정 2026-08-26): AC는 처음부터
  *"소속 인원·**프로젝트**·하위 조직"* 셋을 적었는데 구현은 둘만 봤다. 그 근거였던
  *"프로젝트는 조직 노드를 참조하지 않는다"*는 **셀 방법이 없다**는 뜻이었고, 위 포트가
  그 길을 놓으면서 사라졌다. 이제 `projectCount > 0`이면 `deletable=false`·`409 IN_USE`다.
  **표시와 판정이 같은 수를 읽는다** — 갈라 두면 화면이 "프로젝트 128"이라 적은 노드에서
  삭제가 성공한다. 그래서 완료 프로젝트도 센다(§12 정의의 "삭제되지 않은"이 기준이다).
- **실제로 막히는 경우는 하나다**(실측): 인원 수는 활성만 세므로, 퇴사 처리(E2-3)된 PM만
  남은 노드는 **인원 0 · 프로젝트 N**으로 보인다. 그 노드가 그동안 삭제됐고, 스키마에
  FK가 없어(V1~V16) DB도 막지 않아 사라진 노드를 가리키는 참조만 남았다.
- **부록 B 정정 — `projects.json`의 `team` 문자열은 시드된 DB를 설명하지 않는다**
  (2026-08-26 실측): `managerId`는 실제 명부에 **id로** 붙고 `team`은 구 익명 명부
  기준이라 **382건 중 300건이 어긋난다**. 실 분포는 13개 노드에 걸리고 최대가
  **`AI팀` 128**이다(`AX솔루션사업부` 128이 아니다). §12 등재문이 근거로 든
  "프로토타입 실측 11개 노드 일치"는 프로토타입의 목업 데이터에서 성립한 값이다.
...
- **모듈 간 공개 계약 (모듈 루트에 있는 것 = 밖으로 나가는 전부 — §0)**: 소비자가 실제로 쓰는 좁은 면만 열고, 내부 계약(`ProjectQueryService` 등)은 하위 패키지에 둔 채로 둔다.
  ...
  | **`ProjectCountPort`** | person(정의) | project(구현) | **노드별 프로젝트 수**(E3-1~E3-3 화면 · E3-3 삭제 판정) — 같은 방향 역전. **키가 PM이지 조직 노드가 아니다**: 소속으로 접는 것은 person이 한다(2026-08-26 신설) |
  ...
  | ~~`NotificationService` · `NotifyCommand` 등~~ → **모듈 루트에서 내려갔다**(2026-08-26) | notification | **없음 — 구독자 자신** | 알림 적재. **2026-08-24 정정**: 구 표는 소비자를 `project · resource`로 적었는데, 그러면 §8(발행자 project·resource → 구독자 notification)과 합쳐져 **순환**이 된다. 적재 경로의 정본은 이벤트이고(§8) `notify`는 notification의 리스너가 자기 자신을 부르는 데 쓴다 — 밖에서 부르지 않는다. **2026-08-26 실행**: 소비자가 없으므로 §0의 "모듈 루트 = 밖으로 나가는 전부"에 따라 5종을 `service/`(+`dto/`·`entity/`)로 내렸다 — notification의 루트가 **0**이 됐다(auth·mcp와 같은 모양) |
...
**참조 데이터의 id 발급 규칙 (2026-08-24 — 부록 B에 흩어져 있던 판단을 여기로 승격)**: `Person`·`OrgUnit`·`Grade`·`PermissionGroup`·유지보수 계약/이슈는 `@GeneratedValue` 없는 **명시 id**다(시드가 원본 번호를 그대로 쓴다 — eval 앵커가 생성 순서에 기대지 않게). 새 행의 id는 **하드 삭제가 있는가**로 갈린다.

| | 하드 삭제 | 새 id |
|---|---|---|
| 유지보수 계약·이슈 | 없음(D2-2 — 상태 `종료`로) | `max(id)+1` |
| 조직 노드 · **직급** · **권한 그룹** | 있음(E3-3·E4-3·E5-4) | **시퀀스** |

시퀀스를 쓰는 이유는 실측된 사고다: 조직 노드를 `max(id)+1`로 발급하다가 삭제된 MOIN개발팀(17)의 id를 새 노드가 받아 **그 팀의 비활성 인원 2명이 엉뚱한 조직에 붙었다**(2026-08-22). 인원은 soft 삭제라 참조가 남고 감사 로그는 append-only라 지워진 id를 영원히 가리키므로, 되돌아가지 않는 시퀀스만이 재사용을 원천적으로 막는다. 시퀀스는 **역대 최고값**(살아 있는 행 · 인원이 가리키는 값 · 감사 로그의 entity_id 중 최대) 위에서 시작한다.
...
- **phase = status 파생 그룹** (2026-08-06 · **화면 배치 개정 2026-08-25**): 영업={계약대기,수주확정} · 솔루션={진행중,완료}. 저장 컬럼이 아니라 서버 단일 정의의 파생값(원본 이중화 금지) — 목록 `?phase=` 필터 + 목록·단건 응답 파생 필드(§7). 유지보수 탭은 프로젝트가 아닌 `MaintenanceContract` 목록(§4)이 원천 — status=유지보수중 프로젝트의 화면 노출은 연결된 계약이 담당한다.
  - **화면은 phase 하나당 하나다**(2026-08-25 사용자 결정 — 공용 결정 기록): 한 목록 화면 안의 "전체/영업/솔루션" **탭이 아니라** 내비 항목 둘(**영업** = `?phase=SALES` · **프로젝트** = `?phase=SOLUTION`)이다. 두 화면은 겹치지 않고 **"전체"에 해당하는 화면은 없다** — 그것이 위 문면과 일치한다(유지보수중을 프로젝트 목록에 보여 주던 "전체" 탭이 오히려 이 규칙의 예외였다). 내비 이름이 "솔루션"이 아니라 "프로젝트"인 것은 사용자 어휘를 따른 것이고, 서버 열거 값은 그대로 `SOLUTION`이다.
  - 파생상: **신규 등록 버튼은 영업 화면에만** 있다(신규는 계약대기로 태어나므로 솔루션 화면에 두면 만든 것이 그 목록에 나타나지 않는다) · **상태 칩은 받아 온 행에서 만든다**(phase→status 표를 화면이 갖지 않으려는 것 — 그 표의 사본이 화면에 생기면 위의 이중화 금지가 코드에서 깨진다).
...
- D1-2 Given status≠완료 When 이관 Then `409 INVALID_TRANSITION`, 아무것도 안 바뀜(원자성) — **구 문면 `NOT_COMPLETED`를 2026-08-25 정정**(사용자 결정): 같은 상태 기계의 다른 칸(A7 완료·재개)이 이미 `INVALID_TRANSITION`을 쓰므로 코드를 늘릴 이유가 없고, `ErrorCode`를 늘리면 `mcp` 모듈의 예외 매핑이 함께 움직여 소유 경계를 넘는다
...
**US-D3 유지보수 이슈를 등록·처리한다** [등록·처리·코멘트 추가 = 로그인 사용자 전체 · **정정·삭제 = 등록자·담당자·"계약 관리" 플래그**] (v2.4 신설 — 구 이슈 게시판 대체 · 관문 분화는 v2.31 2026-08-26)
- D3-1 When `POST /api/maintenance/issues {siteId, type, 제목, 본문?}` Then `201` · **assigneeId 기본값 = 해당 사이트 engineerId** · **reporterId = 화자**(2026-08-26 — 정정 권한의 근거) · 담당자에게 알림(`MaintenanceIssueRegistered` — §8). 본문은 **선택**이다(시드 이슈 267건은 제목만 있다)
- D3-2 When `PATCH /api/maintenance/issues/{id}` (상태·담당 재배정, `version`) Then `200` + AuditLog — 상태 흐름 접수→처리중→고객확인대기(선택)→완료, 완료 시 완료일 기록. 역방향은 재개(완료→처리중)만 허용. **이 두 칸은 관문이 없다**(현장의 일이다)
- D3-3 When `POST /issues/{id}/comments` Then `201` — 로그인 사용자 전체. ~~append-only~~ (구 문면: *"수정·삭제 API 없음, 보정은 새 코멘트로만(구 MaintenanceLog 불변식 계승)"* → **2026-08-26 사용자 결정으로 폐기**, D3-7이 대체한다)
- **D3-5** (2026-08-26 신설) When `PATCH /api/maintenance/issues/{id}`에 **제목·유형·본문**이 실려 있음 Then `200` + AuditLog — **등록자·현재 담당자·"계약 관리" 플래그**만 가능하고 그 밖은 `403`. null = 그대로, **본문은 빈 문자열로 지운다**, 제목은 비울 수 없다(`400`). 다섯 칸이 전부 비면 `400`. 사이트 이동은 없다(정정이 아니라 이동이고 AC에 요구가 없다)
- **D3-6** (2026-08-26 신설) When `DELETE /api/maintenance/issues/{id}?version=` Then `200` — **soft 삭제**(프로젝트 A4 선례. 행은 남고 목록·상세·계약 이슈 요약 **전부에서** 빠진다). 권한은 D3-5와 같고(`403`), `version` 어긋나면 `409`, 삭제된 이슈는 이후 조회·수정·재삭제가 **모두 `404`**(삭제와 부재를 구분하지 않는다). 되돌리기 경로는 없다
- **D3-7** (2026-08-26 신설) When `PUT`/`DELETE /api/maintenance/issues/comments/{commentId}` Then `200` — **작성자 본인만**이고 그 밖은 `403`(**"계약 관리" 플래그도 미치지 않는다** — 사용자 결정이 범위를 작성자로 좁혔다). 수정은 `updatedAt`을 남긴다(흔적을 지우지 않는다), 삭제는 행을 지운다(tombstone 미채택). 이슈가 삭제됐으면 그 코멘트도 `404`
...
- E2-2 When `PUT /api/people/{id}` Then `200` — 이름·소속 조직·직급(coeff)·**권한 그룹**(2026-08-09 ⑦ — 그룹 부여는 이 경로)·**`billable`**(2026-08-26 — 부록 B "플래그는 운영 중 개인 단위 수정 가능"에 대응하는 쓰기 경로. 요청 본문의 **필수 필드**이므로 이 값을 빼면 400이다) 변경, AuditLog UPDATE(`billable`도 스냅샷에 든다 — 없으면 그 필드만 바꾼 요청이 감사 행을 남기지 않는다)
- E2-3 When `DELETE /api/people/{id}` Then `200` + `{success:true}`(구 `204`) — soft 비활성(로그인 차단·목록 제외), 과거 배정·감사·집계는 보존
- **E2-3a** (2026-08-26 — §12 ③) When 위 요청 And 대상이 **진행 중 프로젝트에 참여자로 물려 있음** Then 그 배정을 **함께 종료**하고(B2-1 규칙 그대로 — 상태 CLOSED·`endDate` 종료월 말일·감사·`AssignmentChanged`) 응답 `data`에 **무엇이 끊겼는지**를 싣는다(`closedAssignments`·`projects`·`notice`). 살려 두면 퇴사자가 가동률 모집단에 남는다(C1-4)
- **E2-3b** (2026-08-26 — §12 ③) When 위 요청 And 대상이 **진행 중 프로젝트의 PM** Then `409 IN_USE` + 그 프로젝트명(셋까지, 나머지는 "외 N건") — **아무것도 바뀌지 않는다**. PM 공석은 A6-5가 금지하고 교체 대상은 시스템이 고를 수 없다(US-A6로 교체한 뒤 다시 삭제한다)
- **E2-3c** (2026-08-26) "진행 중"은 **배정이 살아 있고 프로젝트도 진행 중**(계약대기·수주확정·진행중)이라는 뜻이다 — **완료·유지보수중 프로젝트의 배정은 세지도 끊지도 않는다**. 같은 정의를 E1-2 이동 경고가 공유한다(`ProjectStatus.LIVE` 한 벌)
...
- E3-3 Given 소속 인원·프로젝트·하위 조직 중 하나라도 존재 When `DELETE /api/org-units/{id}` Then `409 IN_USE`, 아무것도 안 바뀜 — 빈 노드만 삭제 가능. **"프로젝트"는 2026-08-26부터 실제로 센다**(사용자 결정 — 그전에는 셀 경로가 없었다): 기준은 목록의 `projectCount`와 **같은 수**이고(그 노드가 PM 소속 노드인, 삭제되지 않은 프로젝트), 인원은 활성만·프로젝트는 비활성 PM의 것까지 센다 — 퇴사자만 남은 노드가 "인원 0"으로 보여 지워지던 자리다
- E3-4 Given TEAM scope 사용자의 소속 노드에 하위 조직 존재 When 목록 조회 Then 하위 조직(subtree) 인원·프로젝트 포함 — 가시성 subtree 검증(상위 §4-4)
- E3-5 When `PUT /api/org-units/{id}/parent {parentId}` Then `200` + AuditLog UPDATE(before/after `parentId`) — **노드 이동**(2026-08-24 신설, 사용자 결정). 소속 인원·프로젝트는 `orgUnitId` 참조라 노드와 함께 옮겨지고 가시성은 다음 요청부터 새 경로를 따른다(E3-2 개명과 같은 원리 — 비정규화 컬럼이 없다). 신설 근거: 조직 개편이 실제로 발생하는데(2026-08-24 검수 — `경영관리팀`을 신설 `관리•마케팅부` 아래로) 트리에 생성·개명·삭제만 있어 **개편을 화면에서 할 수 없었다**. 부문 이동은 `PersonRef.division` 파생값을 바꾸므로 프로젝트·가동률 응답의 부문 표시가 함께 움직인다(같은 트리를 한 규칙으로 읽는다 — `OrgTree.topDivisionIdOf`)
- E3-6 Given 대상이 자기 자신 또는 자기 subtree 안의 노드 When 이동 Then `400 VALIDATION_ERROR` field=parentId, 아무것도 안 바뀜 — 순환은 트리를 트리가 아니게 만들고, 가시성·부문 계산이 무한히 돈다. Given 회사(root) When 이동 Then `400 VALIDATION_ERROR` — root는 정의상 부모가 없고(§4) 부문 가시성이 "root 직계 자식"이라 root가 남의 아래로 들어가면 그 계산이 무너진다. Given 없는 상위 조직 When 이동 Then `422 REF_NOT_FOUND` (E3-1 생성과 같은 판정을 공유한다)
...
- **SSE 인증**: EventSource는 헤더를 싣지 못하므로 `?access_token=` 쿼리 파라미터로 인증 — 액세스 로그에 토큰이 남지 않도록 마스킹 필수. **인증 실패는 스트림 라우트가 직접 401을 낸다**(2026-08-25 실측): 전역 핸들러의 JSON 봉투는 이 요청의 `Accept: text/event-stream`과 협상이 깨져 401이 아니라 500이 됐다. EventSource는 상태 코드만 읽으므로 본문 없는 401이 맞다.
...
| `ProjectCompleted` | project | notification | 완료/이관 안내 (발행 시점: `/complete` 커밋 후 — US-A7). **2026-08-25 배선**: 명세만 있고 발행 지점이 0곳이었다 — `ProjectLifecycleChanged(COMPLETED)`로 발행한다 |
| `ProjectReopened` | project | notification | 완료 지연 알림 회수 (F3-3). **2026-08-25 배선**: `withdrawUnread`는 구현·테스트돼 있었으나 부르는 자리가 없어 재개해도 회수되지 않았다 — `ProjectLifecycleChanged(REOPENED)`로 발행한다 |
...
| `ProjectReminderDue(DEADLINE_NEAR)` | project | notification | 종료일 D-7 진행중 프로젝트의 PM에게 마감 임박 알림 (F2-1 — **2026-08-25 신설**: §8이 알림 *유형*만 정하고 만들어지는 경로를 비워 뒀는데, "notification 밖에서 `notify`를 부르지 않는다"는 규칙상 스케줄러도 발행해야 한다) |
| `ProjectReminderDue(COMPLETION_OVERDUE)` | project | notification | 100%인 채 7일 경과한 프로젝트의 PM·PL에게 완료 지연 알림 (F3-1 — 2026-08-25 신설, 위와 같은 이유) |
...
| EPIC | 상태 | 남은 것 |
|------|------|---------|
| **A 프로젝트** | 대부분 실구현 — A1 생성 · A2 진척률 2단계 · A3 조회·가시성 · A4 삭제 · A5 수정·상태전이 · A6-1·2·4·5 PM 교체 · **A6-3·A6-6·A6-7 역할 지정**(2026-08-24 — `PUT /projects/{id}/roles`, 화면은 배정 목록의 드롭다운+확인) · A7 완료·재개 · **`?phase=` 목록 필터 + phase 탭**(2026-08-25 — `ProjectPhase`가 status 집합을 들고 양방향으로 읽힌다: 단건은 status→phase, 목록 필터는 phase→status 집합. 목록 응답에도 파생 `phase`를 실어 화면이 표를 갖지 않게 했다) · **A8 권한 커스텀 전량**(2026-08-26 — `GET`/`PUT /projects/{id}/permissions`, Flyway **V16**. 표는 `ProjectPermissionRules` 한 벌이고 병합은 `EffectiveProjectPermissions` 한 곳이라 **판정과 조회가 같은 답을 본다**. 저장은 전체 교체이며 기본값과 같은 칸은 행을 남기지 않는다 — "행이 없다 = 기본값"이 불변식이라 §4-2 표가 바뀌어도 데이터가 옛 기본값을 들고 있지 않는다. A8-5·A8-6이 요구한 침투는 `ProjectActionPermission.require`가 병합 결과를 읽는 것으로 이뤄졌고 **호출부는 한 줄도 바뀌지 않았다**) | **없다.** A8-1~A8-7 전부 |
...
| **D 유지보수** | **전량 실구현**(2026-08-25 — EPIC D가 닫혔다). **조회**(2026-08-23) 모듈·엔티티 4종·Flyway V9·시드 적재 · D4-1 목록(keyword 3종) · D4-2 상세 · D4-3 전사 공개 · D3-4 이슈 조회. **쓰기 D2**(2026-08-24) 계약 직접 등록·수정(낙관적 락 · 삭제 API 없음)·사이트 등록·수정 · D2-3 "계약 관리" 플래그 403. **쓰기 D3**(2026-08-24) 이슈 등록(담당 기본값 = 사이트 engineerId)·상태 전이(`IssueStatus.canTransitionTo`, 흐름 밖은 409)·append-only 코멘트 — **관문 없는 쓰기 계약**(US-D3 = 로그인 사용자 전체). **이관 D1**(2026-08-25) `POST /projects/{id}/handover` — 계약·사이트 생성과 완료→유지보수중 전이가 **한 트랜잭션**(D1-2·D1-3 원자성을 통합 테스트가 실물 행 수로 실증), 권한은 `ProjectAction.HANDOVER`(PM만), 커밋 후 `MaintenanceHandedOver` 발행. **모듈 방향은 역포트**(`project.HandoverPort` — project 정의, maintenance 구현. 2026-08-25 사용자 결정): 실측상 두 모듈은 서로를 모르는 형제였고 잠재 의존이 maintenance → project를 가리켰다. 어댑터는 `ContractCommandService`를 재사용하지 않는다 — 그 관문을 지나면 "계약 관리" 플래그 없는 PM이 자기 프로젝트를 이관할 수 없다 | 없다. **이슈 정정·삭제까지 섰다**(2026-08-26 — D3-5·D3-6·D3-7 신설, Flyway **V17**: 본문·등록자·soft 삭제 플래그·코멘트 `updatedAt`. 착수 계기는 실측된 공백이다 — 등록 경로는 있는데 **제목 오타를 고칠 방법이 아예 없었다**. 관문이 **행위별로** 갈린다: 등록·처리·코멘트 추가는 전원, 정정·삭제는 등록자·담당자·"계약 관리" 플래그, 코멘트 수정·삭제는 **작성자 본인만**). 화면도 전부 붙었다(계약·사이트 쓰기 · 이슈 등록·처리·코멘트·**정정·삭제** · 이관 폼). 파생 공백 2건 등재: **재배정은 알리지 않는다**(§8에 이벤트 없음) · **이관 알림 수신자를 구현이 정했다**(§8 미명세 — 사이트 담당 엔지니어) |
| **E 조직·사용자** | **전량 실구현**(2026-08-24) — E1-1 소속 이동(+E1-2 경고) · E2-1 등록 · E2-2 수정 · **E2-3 비활성 — 2026-08-26부터 배정까지 든다**(E2-3a 참여자 배정 자동 종료 + 안내 · E2-3b PM은 409로 거절 · E2-3c "진행 중"의 정의. §12 ③ 해소 — `AssignmentReleasePort` 신설, 판정은 person·실행은 project) · E2-4 플래그 403 · E2-5 시스템 계정 422 · E3-1 생성 · E3-2 개명 · **E3-3 `IN_USE` 삭제 거절 — 2026-08-26부터 프로젝트도 센다**(AC 문면이 처음으로 전부 성립. 같은 날 `OrgUnitView.projectCount` 신설 — `ProjectCountPort`) · E3-4 subtree 가시성 · **E3-5 이동 · E3-6 순환·root 거절**(2026-08-24 신설) · **E4 직급 3종** · **E5 권한 그룹 3종** · **E2-2가 `billable`도 받는다**(2026-08-26 — §12 ④ 해소. 부록 B의 "운영 중 개인 단위 수정 가능"에 대응하는 쓰기 경로가 그전까지 하나도 없었다. 감사 스냅샷에도 넣었다) | 없다(화면도 붙었다 — 2026-08-24. **인력 목록의 권한 그룹 열은 2026-08-26** — 관리 권한자 전용이고 서버 DTO는 넓히지 않았다). 부수 2건: ①직급·권한 그룹 id를 **시퀀스**로 발급(Flyway V11) ②**낙관적 락 결함 수정**(2026-08-24) — E2-2·E4-2·E5-2가 요청의 `version`을 받아만 두고 검사하지 않아 마지막 쓰기가 조용히 이기고 있었고, 응답의 version도 flush 전 값이었다. 목록 반환형이 `ReferenceItem` → `GradeDetail`·`PermissionGroupDetail`(+`memberCount`)로 넓어졌다 — §7에 상세 라우트가 없어 목록이 곧 관리 화면의 원천이다 — 하드 삭제가 있는 명시 id 참조 데이터의 규칙(부록 B)을 따랐다 |
...
| 시드 적재 | **전량 적재된다** (2026-08-26 실측 정정 — 이 행이 stale이었다): 부팅 시 로더 3종이 조직·직급·권한 그룹·인원·User(`seed_org_proten.sql` 44명) + **`projects.json` 382건** + **`maintenance.json` 계약 105건**을 채운다(이미 있으면 "적재 생략"을 로그로 남긴다). 구 문면은 "`projects.json`·`maintenance.json` 미적재"였는데 그 뒤 v2.10·v2.12가 로더를 넣었고 이 행만 따라오지 않았다 |
| 프론트엔드 (`frontend/`) | **구현된 백엔드에 화면이 전부 붙었다(2026-08-24)** — 라우트 전량 대조로 "서버에 있는데 화면이 없는" 9지점을 찾아 연결했다: 알림 벨·읽음(F1-3)·알림 설정(H1-4) · 인력 수정(E2-2)·소속 이동+경고(E1-1·E1-2) · 조직 개명(E3-2) · 직급 관리(E4) · 권한 그룹 관리(E5) · 유지보수 계약·사이트 쓰기(D2). 그 전에 붙은 것: 프로젝트 전 경로 · 가동률(C) · 유지보수 조회(D) · 감사 2뷰(G). **"있는 엔드포인트만 부른다"는 원칙은 그대로다.** 그 뒤로 서버가 선 것마다 화면이 따라 붙어(SSE F1-4 · D1 이관 · D3 이슈 쓰기 · A6-3 · `?phase=` · **A8 권한 패널** 2026-08-26) **지금은 화면이 없는 서버 기능이 없다**. **부록 A와의 차이 3건**: ①통합 감사 로그를 `/settings` 3탭이 아니라 별 항목으로 뒀고 직급·권한 그룹 관리도 그 배치를 따라 `/people`에 붙였다(§12) ②알림은 부팅 로드 + **SSE 스트림**이다(2026-08-25 F1-4 — 벨 열기 재조회를 없앴다) ③프로젝트 목록이 **단계 탭이 아니라 내비 항목 둘**(영업·프로젝트)이다(2026-08-25 사용자 결정 — §5). **A8 권한 패널의 파급 1건**: `permissions.ts`가 갖고 있던 §4-2 사본을 걷었다 — 프로젝트별 커스텀이 들어오면 그 사본은 override가 걸린 프로젝트에서 **틀린다**(끈 기능의 버튼이 그대로 보인다). 이제 화면은 상세와 함께 받은 매트릭스의 `allowed`를 읽는다 |
| `/mcp` 어댑터 | **이 앱의 8번째 모듈**(2026-08-23 재승격 — MCP 담당 소유, §3). **의존 방향 확정: `mcp` → 도메인 루트 한 방향**(구조 원칙 3 — 어댑터가 애플리케이션 서비스를 호출한다). 각 도메인이 자기 모듈 루트에 조회 계약을 올리고 어댑터는 그것만 부른다 — port 인터페이스를 `mcp` 루트에 두고 도메인이 구현하던 M0 방식은 폐기했다. 도구 **8종 전량 실연결**(2026-08-26 실측 정정 — 이 행이 "6종"에 멈춰 있었다. 가동률 2종은 2026-08-24에 배선됐고 `/mcp`에 FR-AI-26 503으로 답하는 자리가 0건이다. 기동 로그 `Registered tools: 8` · `tools/list` 8종 확인) |
...
| `/settings` | 설정 (2026-08-09 ⑤⑦⑧ — 3탭, 프로토타입 3차 반영 구조) | **[사용자 관리]** 사용자 CRUD(US-E2 — 그룹 부여 포함)·시스템 계정은 수정/삭제 비활성(E2-5) · **[조직 관리]** 조직 트리(좌 — US-E3: +하위/개명/**이동**(E3-5 — 2026-08-24 신설)/삭제, 노드별 인원·프로젝트 수) + 직급 관리·권한 그룹 관리(우 — US-E4·E5: 그룹 행 = 뱃지·설명·n명·[권한 ▾] 펼침(가시성 select+기능 토글)·[수정]·인원 0일 때만 [삭제], 관리자 그룹은 버튼 비활성) · **[감사 로그]** 통합로그 조회(G1-3) | 그룹: 사용자/조직/권한 관리 |
| `/sales` | **영업 목록** (2026-08-25 신설 — 내비 항목 둘 중 하나) | `?phase=SALES`(계약대기·수주확정) · 상태 칩·이름 검색 · **(생성 권한자) 등록 버튼은 이 화면에만**(신규는 계약대기로 태어난다) | 가시성 범위 |
| `/projects` | 프로젝트 목록 | `?phase=SOLUTION`(진행중·완료) · 상태 칩·제품군(solution) 필터 · 이름 검색 · 페이지네이션 · 완료 건은 "이관 대기" 뱃지 | 가시성 범위 |
| `/projects/new` | 프로젝트 등록 | 입력 항목 폼 + 참여자별 role(**PM/PL/참여자**) 선택, PM 1명 필수 · 수행형태 3종(원격·상주·부분상주 — 2026-08-09 ③⑥) · 422/409 오류 표시 | 그룹: 프로젝트 생성 |
| `/projects/:id` | 프로젝트 상세 | 기본정보 · 상태 뱃지 · **(PM·PL) 상태 전이 버튼 — 다음 한 칸만(계약대기→수주확정→진행중) + 확인 카드. 되돌릴 수 없으므로 조회 화면에서 확인과 함께 수행하고, 정보 수정 폼에서는 status를 다루지 않는다(2026-08-22 결정)** · 진행률(권한 시 수정 — **100% 저장만 확인 모달, 그 외 1클릭** 2026-08-09 ①. 서비스·MCP 2단계는 무변경) · 배정 목록(**역할 뱃지 PM/PL/참여자**) · lastEditedBy/At · (PM) 역할 변경 UI — **배정 목록의 역할 드롭다운 + [수정] + 확인 모달**(2026-08-24 사용자 결정: PM 지정은 직전 PM을 참여자로 내리는 부수효과가 있어 고르는 순간 저장되면 무엇이 함께 바뀌는지 볼 자리가 없다. PM 행에는 드롭다운을 두지 않는다 — PM을 내리는 요청은 서버가 422로 막는다) · (배정 인원, 100% 시) **완료 처리 버튼** · (배정 인원, 완료 시) **재개 버튼**(US-A7) · (PM, 완료 시) 이관 버튼 · (PM) **권한 패널**(역할×기능 토글 매트릭스 — US-A8. 고정 셀은 잠금 표시, 기본값과 다른 셀은 커스텀 뱃지, **기본값 복원** 버튼. 완료·재개는 한 토글) · **이력 탭**(프로젝트 스코프 변경 이력 — US-G2, 가시성 범위 전체. lastEditedBy/At의 상세판) | 가시성 범위 |
...
- 직급계수(gradeCoeff): 대표이사 2.0 · 부대표 1.8 · 상무 1.7 · 이사 1.6 · 수석 1.5 · 책임 1.2 · 선임 1.0 · 주임 0.8 · 수습 0.5
...
- **`OrgUnit` 트리 — (2026-08-09 ⑧ · **2026-08-24 재편**: 노드 17 → **18**. 사용자 제공 조직도 기준으로 `관리•마케팅부`(부문)를 신설해 `경영관리팀`을 그 아래로 내리고, 대표를 회사 직속으로 옮기고, `AI개발팀`을 실제 이름 `AI팀`으로 되돌렸다 — `projects.json`이 이미 `team="AI팀"`을 쓰고 있었다. 노드 id는 보존한다(경영관리팀 = 2) — 인원·감사 로그가 id로 가리킨다. 이미 적재된 DB는 Flyway V14가 수렴시킨다)**: 시드 부문·팀 2단을 회사(root) 아래 그대로 적재 — 임의 깊이는 운영 중 US-E3로 확장(시드에 3단 이상 데이터 없음)
...
- `Person.billable` — **false 목록 확정(2026-08-10 · 실제 명부로 재앵커 2026-08-24)**: **조직 단위 2곳 = 관리•마케팅부 subtree 전체 3명 + 대표 1명**(2026-08-24 조직 재편 반영 — 사용자 제공 조직도대로 `관리•마케팅부`(부문) → `경영관리팀`(팀)으로 갈리고 대표는 회사 직속이 됐다. **대상 인원은 재편 전과 같은 4명**이고, 대표만 조직 단위로 표현할 수 없어 개인 플래그로 남는다) **· AX사업기획부 전체 6명**(부문 직속 1 + AX영업팀 2 + AX기획마케팅팀 3), 계 **false 10명 + 시스템 계정 1 / true 33명**(구 문서 "34명 true"는 시스템 계정을 true로 세던 값이다). 근거: 세 부문은 진행중 프로젝트 수행이 거의 없다. **예외는 1명이 아니라 5명이다**(2026-08-24 실측 — 장대근 24건·윤종헌 12·천용우 9·진희원 7·김주선 5): 지원조직 인원의 간헐 참여는 개인 지정 조회로 보이고 **집계는 `billable=true`만 세므로 수치에 영향이 없다** — 플래그는 조직 기준을 유지한다. AI·AX연구소·MS사업부는 수행 이력 있는 딜리버리 조직이라 true — 소속원 0%는 "실제 여유"로 의미 있는 0(플래그는 운영 중 개인 단위 수정 가능)
...
- **검수 기획 결정 4건 중 1건이 남았다 — ②는 2026-08-25, ④와 ③은 2026-08-26 구현 완료 (등재 2026-08-24 · 공용 결정 기록 참조)**: ①**영업 단계(계약대기·수주확정)는 PM 없이 생성한다** — A1-4의 "PM 1명 필수"를 상태별 조건부로, `manager_id`를 nullable로, A6-5 불변식("프로젝트당 role=PM 정확히 1행")을 **진행중 이후에만** 성립하게 재작성해야 한다. 수주확정→진행중 전이의 PM 필수 AC가 신설되고(화면 버튼도 "인원 배정 →") **MCP 접점이 함께 움직인다**(`search_projects` 응답의 PM 필드 null 가능 → 도구 description·프롬프트·eval 한 세트 — MCP 담당 확인 필요). PM 공석 프로젝트의 쓰기 주체는 **새 권한 그룹**(`manageAllProjects`+`createProject`)이 맡는다 — `createdBy` 신설·새 치환 규칙은 채택하지 않았다. ~~②**phase 탭 + `?phase=` 서버 필터** — 접근 제한은 두지 않는다(전원 공개, 사용자 철회)~~ → **2026-08-25 구현 완료**(§10 A행. 결정대로 접근 제한 없음 — 필터는 가시성 판정 **뒤에** 얹히므로 범위를 넓히지 못한다). ~~③**인원 삭제 시 진행 중 프로젝트 목록 안내 + 배정 자동 종료, 단 PM 배정은 종료하지 않고 교체를 요구한다**~~ → **2026-08-26 해소**(AC E2-3a·E2-3b·E2-3c 신설): `person.AssignmentReleasePort`(person 정의 · project 구현) — **`AssignmentCountPort`를 넓히지 않고 포트를 나눴다**(2026-08-26 사용자 결정). 등재문은 넓히라고 적었지만 그 계약의 javadoc이 자기 면을 "E1-2 경고에 필요한 건수 하나"로 좁힌 근거를 이미 적어 뒀고, 넓히면 경고가 쓰지도 않는 필드에 person이 의존한다 — 전날 `ProjectCountPort`가 같은 판단을 한 번 더 했으므로 **질문마다 포트를 나눈다**가 규칙이 됐다. 판정(PM이면 거절)은 person이 하고 실행(종료·감사·이벤트)은 project가 한다. 어댑터는 `AssignmentService.close`를 **재사용하지 않는다** — 그 관문은 프로젝트별 `ASSIGN`(=PM) 권한이라 관리자가 남의 프로젝트 PM이 아니라는 이유로 퇴사 처리를 못 하게 된다(D1 이관 어댑터와 같은 판단). **착수 전 실측이 선행 결함을 하나 드러냈다 — 아래 항목.** ~~④**`Person.billable` 토글**~~ → **2026-08-26 해소**: E2-2(`PUT /api/people/{id}`)가 이 필드를 받는다. **구 주석이 근거를 잘못 읽고 있었다** — `Person.update`가 "부록 B가 조직 단위로 정한다"며 뺐는데, 그것은 부록 B의 **시드 적재 기준**이고 같은 문단이 "(플래그는 운영 중 개인 단위 수정 가능)"으로 끝난다. 적재는 조직 단위, 운영 중 개인 예외는 이 경로다. **관문은 그대로 "사용자/조직/권한 관리"**(집계 모집단을 정하는 일은 조직 운영이고 그 플래그가 이미 그 권한이다) · **감사 스냅샷에 `billable`을 넣었다**(없으면 그 필드만 바꾼 요청의 diff가 비어 감사 행이 0건이 된다 — H1-2의 email이 같은 자리였다) · `capacity`는 여전히 닫아 뒀다(월별 `Capacity`가 우선하는 **분모**라 판단 축이 다르다)
- ~~**"진행 중 배정"이 진행 중을 세지 않았다 (2026-08-26 등재·같은 날 해소 — §12 ③의 선행)**~~ → **해소**: `AssignmentCountPort`가 세는 기준이 **배정 상태 하나**뿐이라 완료 프로젝트의 배정이 그대로 걸렸다. **실측**: 시드된 DB에서 ACTIVE 배정 462건 중 **384건(83%)이 완료 프로젝트**의 것이고, 한 사람은 128건 중 진행 중이 **5건**이었다 — E1-2 이동 경고가 "진행 중인 배정 128건"이라 답하고 있었다(25배). **근거 문장이 사실이 아니었다**: 어댑터 javadoc이 *"완료 시 배정이 이미 종료 처리된다(AC B2-1)"*고 적었는데 B2-1은 `DELETE /assignments/{id}`, 즉 **명시적 수동 종료**이고 완료 시 자동 종료를 정한 AC는 어디에도 없다. **고친 방향은 판정이다**(2026-08-26 사용자 결정 — 데이터 무변경): 자동 종료를 만들면 종료 경로가 B2-1과 둘로 갈리고 재개(A7-3)에 되살리기 규칙이 붙는다. 정의는 `ProjectStatus.LIVE` **한 벌**이고 이동 경고(E1-2)와 퇴사 처리(E2-3a)가 같이 읽는다. **파급은 그 경고 하나로 좁혀졌다** — 가동률은 기간 겹침으로 세므로(C1-1) 완료 건이 애초에 안 걸린다(2026-08 겹침 중 COMPLETED 0건, 실측). **부수 1건**: 퇴사가 배정을 끊으면 그 `AssignmentChanged`를 받은 `OverbookingDetector`가 **비활성 인원의 가시성을 물어 404로 터졌다**(화자가 대상 본인이라 `RequesterResolver`가 활성만 찾는다) — 비활성 인원을 건너뛰게 고쳤다. 비동기라 트랜잭션은 멀쩡했고 **로그만 남던 자리**다
- **이슈 본문이 `list_maintenance_logs`에 실리지 않는다 — MCP 담당 확인 요청 (2026-08-26 등재)**: D3-1·D3-5로 이슈에 **본문**이 생겼는데 도구 응답(`MaintenanceLogsResult.IssueView`)은 `id·type·status·title·receivedAt·assignee·comments`뿐이라 그 칸이 없다. **웹은 내용을 보는데 챗은 못 보는 상태**이고, 그것은 이 프로젝트가 반복해서 경계한 "두 경로가 같은 답을 내지 않는다"의 한 자리다(가동률 `scope=COMPANY`·`search_projects` 배정 플래그와 같은 종류). 다만 **G1을 막지 않는다**: eval 36케이스에 이슈 본문을 묻는 것이 없고(C류는 계약·이슈 이력 조회다) 시드 이슈 267건은 본문이 **전부 null**이라 지금 실을 값도 없다. 실을지·description을 어떻게 고칠지는 **카탈로그 결정**이라 MCP 담당 소유다(상위 PRD §6 — 양측 합의). pms 쪽은 `IssueView.content`가 이미 서 있어 어댑터가 읽을 면은 준비됐다. **삭제분은 이미 도구에서도 빠진다**(저장소 질의가 `deleted=false`를 든다 — 계약 상세 이슈 요약도 같다) — 그쪽은 계약 변경이 아니라 데이터 정합이라 확인 대상이 아니다
- **A8-4 문면 이탈 — 모르는 기능 이름은 422가 아니라 400이다 (2026-08-26 등재 · 구현이 정했다)**: A8-4는 *"고정 셀(role=PM · action=**조회/삭제**/이관) 포함 요청 → 422 IMMUTABLE_PERMISSION"*이라고 적지만, **조회·삭제는 `ProjectAction` 값이 아니다** — 조회는 가시성의 몫이고(`requireVisible`) 삭제는 판정 축이 둘이라 `requireDelete`가 따로 든다(§4-2 고정 행). 그래서 그 이름을 담은 요청은 스프링 열거 변환이 실패해 **§7 봉투 400**으로 답한다. **"없는 칸"과 "못 바꾸는 칸"은 다른 답이어야 한다**는 판단이고(`?phase=` 선례 — 모르는 값은 400), 실재하는 고정 칸(PM 열·이관 행)은 문면대로 **422 IMMUTABLE_PERMISSION**이다. 두 갈래 다 아무것도 바꾸지 않으므로 AC의 관측 요구("아무것도 안 바뀜")는 성립한다. 컨트롤러 테스트가 두 갈래를 각각 잠근다
- **파생 필드로는 정렬할 수 없는데 §7이 `?sort=`를 일반 규칙으로 적는다 (2026-08-25 등재 — 미해결)**: 목록 응답이 파생 `phase`를 실어 주므로 호출자가 `?sort=phase,asc`를 시도할 법한데, 영속 컬럼이 아니라 스프링 데이터가 `PropertyReferenceException`을 내고 전역 핸들러의 catch-all이 **500**으로 답한다. 잘못된 sort 키 전반의 기존 성질이고(파생 필드에 국한되지 않는다) §7에 그 오류 규칙이 없어 이번 범위로 다루지 않았다 — 고칠 때는 "정렬 가능한 키" 목록이 어디에 사는지가 먼저다
- ~~**"전체" 탭의 구별력을 실 데이터로는 볼 수 없다** (2026-08-25 등재 — 실측)~~ → **같은 날 화면 분할로 해소**: "전체" 탭 자체가 없어졌다(§5 화면 배치 개정 — 내비 항목 둘). 그 등재의 관측은 남겨 둘 값이 있다 — 시드 382건의 분포가 완료 319 · 진행중 34 · 수주확정 19 · 계약대기 10이고 **유지보수중이 0건**이라 영업 29 + 솔루션 353이 정확히 382다. 즉 **어느 화면에서도 유지보수중 프로젝트를 볼 수 없는 상태가 실 데이터로는 확인되지 않는다**(볼 것이 없다). 이관이 §5 전이 밖의 전용 경로라 시드가 그것을 재현하지 않는 것이 원인이고 결함은 아니다 — 화면 시연에는 이관을 한 번 일으켜야 한다(부록 B 시연 앵커 = 명화공업). 동작은 통합 테스트가 픽스처로 잠갔다(이관한 프로젝트가 두 phase에서 함께 빠진다)
- ~~**노드별 프로젝트 수가 `OrgUnitView`에 없다 (2026-08-24 등재)**~~ → **2026-08-26 해소**: `person.ProjectCountPort`(person 정의 · project 구현) + `OrgUnitView.projectCount`. 정의는 등재문 그대로다(**그 노드가 PM 소속 노드인, 삭제되지 않은 프로젝트 수** — 직속 기준). **포트의 키를 조직 노드가 아니라 PM으로 둔 것이 설계의 전부다**: "누가 어느 노드에 속하는가"는 person의 지식이라, 노드별로 접어 달라고 하면 같은 규칙이 project에도 생긴다(소속을 바꾸는 경로 E1-1·E3-5가 전부 person 쪽에 있다). 함께 **E3-3의 문면이 처음으로 전부 성립했다**(사용자 결정 — 위 v2.29 절). **다만 등재문의 근거 한 줄은 틀렸다**(2026-08-26 실측): *"프로토타입 실측으로 `projects.json`의 `team` 카운트와 11개 노드 전부 일치했다"*는 **프로토타입의 목업 데이터**에서 성립한 값이다. 실 앱은 `managerId`를 실제 명부에 **id로** 붙이는데 `team` 문자열은 구 익명 명부 기준이라 **382건 중 300건이 어긋난다** — 실 분포는 **13개 노드**이고 최대가 `AI팀` 128이다(`AX솔루션사업부` 128이 아니다). `ProjectSeedLoadIntegrationTest`가 그 13개 값을 앵커로 잠갔다. 이것은 아래 "인물 이름 재매핑" 미해결 항목이 **문서 이름을 넘어 시드 데이터에까지 미친다**는 뜻이다
- ~~**인력 목록의 권한 그룹 컬럼 (2026-08-24 등재)**~~ → **2026-08-26 해소 — DTO를 넓히지 않고 화면 매핑으로 갔다**: 등재문은 *"비관리자에게 보이려면 DTO에 `groupName`을 실어야 한다 — 권한 정보를 어디까지 노출할지가 먼저다"*였는데, **그 판단은 `PersonSummary`의 javadoc에 이미 있었다**(*"그룹 이름을 인력 목록에 실으면 가시성만 있으면 남의 권한 등급이 읽히므로 화면에 필요한 최소만 준다"*). 그래서 **비관리자 노출은 채택하지 않았다** — 부록 A가 요구한 열은 **관리 권한자에게만** 보이고, 화면이 이미 받아 둔 `GET /api/permission-groups`로 id → 이름을 해석한다. **경계를 지키는 것은 서버다**: 그 라우트가 관리 플래그 없이는 403이라(`PermissionGroupServiceImpl.list`) 비관리자에게는 이름이 갈 길이 없다. 화면이 `me.manageOrg`일 때만 부르는 것은 그 위의 편의일 뿐 방어선이 아니다 — **그 읽기 403은 테스트가 없어서 이번에 신설했다**(쓰기 셋만 잠겨 있었다). 부수로 `billable=false` 인원에 "집계 제외" 표시를 같은 열에 붙였다(부록 B · C1-5)
...
- ~~**H1-4 알림 설정의 단위가 `NotificationType`과 맞지 않는다**~~ — **2026-08-24 같은 날 해소**: 설정 단위를 `NotificationType` 5종으로 확정하고 H1-4 문구를 고쳤다(구 `{progress, project, org, weekly}`는 알림 유형이 정해지기 전의 이름이다). 근거는 `NotificationType`의 javadoc이 이미 자기를 설정 단위로 선언하고 있었다는 것. 원 항목: : H1-4는 `PUT /api/me/notif-prefs {progress, project, org, weekly}` 네 칸인데, `NotificationType`은 다섯이다(ASSIGNED·OVERBOOKED·PROJECT_COMPLETED·DEADLINE_NEAR·COMPLETION_OVERDUE). 어느 유형이 어느 칸에 드는지 정의된 적이 없고, `weekly`는 유형이 아니라 **주기**로 읽힌다(그런 알림은 §8에 없다). F1-5(설정 꺼짐이면 적재도 푸시도 안 함)는 이 대응이 정해져야 구현된다 — 지금은 **필터를 항상 통과**시킨다. 세 갈래: ①네 칸을 유형에 대응시킨다 ②설정 단위를 `NotificationType` 다섯으로 바꾼다(H1-4 문구 수정) ③주기(`weekly`)는 F2·F3 스케줄러와 함께 다시 본다
- ~~**notification 루트 계약 5종의 자리 (2026-08-24 등재 — 당시 제목은 "4종"이었다 — `NotificationPreferences`가 H1-4로 늘어난 뒤에도 제목만 옛 수를 들고 있었다)**~~ — **2026-08-24 판정 완료(같은 날)**: `service/`로 내리는 것이 맞다. D-b(D3-1 이슈 등록 알림)가 "정말 아무도 안 부르는가"의 마지막 반증 기회였고 **반증되지 않았다** — D3-1은 maintenance가 `MaintenanceIssueRegistered`를 발행하고 notification이 구독하는 형태여서 외부 호출이 하나도 생기지 않았다(실측: notification 밖에서 이 5종을 import하는 자리 **0건**, 유일한 grep 히트는 javadoc 문장이다). 2026-08-22 리뷰가 이 넷을 루트로 올린 근거("다른 모듈이 부른다")는 §8 이벤트 방향 확정으로 이미 뒤집혔고, 이번에 그것이 새 구독자에서도 성립함이 확인됐다. **이동은 별 단위로 남겼고 2026-08-26에 실행했다**(notification 루트 5종 → service/·dto/·entity/. 루트가 0이 됐고 외부 import는 여전히 0건 · 테스트 628 불변 — 순수 리팩터다). 당시 판단은 이랬다: 순수 리팩터라 이슈 쓰기 작업의 성격과 다르고, 5개 파일 + import를 옮기는 diff를 기능 변경과 섞으면 리뷰가 둘 다 못 본다. 원 항목: 이벤트 한 방향이 확정되며 밖에서 `notify`를 부르는 모듈이 없어졌다 — 그러면 *"모듈 루트 = 밖으로 나가는 전부"*(§0) 규칙상 `NotificationService`·`NotifyCommand`·`NotificationType`·`NotificationView`(+`NotificationPreferences`)는 `service/`로 내려가야 맞다
- **이슈 담당이 바뀌어도 알림이 없다 (2026-08-24 등재 — 미해결)**: D3-2 재배정은 알림을 만들지 않는다. §8에 그 이벤트가 없고 AC도 등록(D3-1)만 알림을 요구했기 때문이며, `AssignmentChanged`에서 수정·종료를 알리지 않는 것과 같은 자리다(AC에 없는 규칙을 구현이 지어내지 않는다). 다만 **결과가 어색하다**: 담당이 자기에게 넘어온 사람은 화면을 봐야 알게 되는데, 유형 이름이 `ISSUE_ASSIGNED`("이슈 담당 지정")여서 더 그렇게 읽힌다. 해소 방향 둘: ①§8에 `MaintenanceIssueReassigned`를 신설한다 ②`MaintenanceIssueRegistered`를 `AssignmentChanged`처럼 `Kind`를 가진 한 타입으로 넓힌다(등록·재배정). **급하지 않다** — eval 36케이스는 이슈 쓰기를 묻지 않는다
- **재완료는 조용하다 (2026-08-25 등재 — 미해결)**: 완료 안내의 멱등 키가 `completed:{projectId}:{recipientId}`라 **완료 → 재개 → 재완료**에서 두 번째 완료 안내가 발행되지 않는다(첫 행이 남아 있어 멱등이 막는다). 재개가 회수하는 것은 `COMPLETION_OVERDUE`뿐이다. §8·§12 어디에도 이 경우가 적힌 적이 없어 의도인지 누락인지 정해진 바가 없다 — 해소 방향 둘: ①재개 시 완료 안내도 함께 회수한다(그러면 재완료가 다시 알린다) ②키에 완료 회차를 넣는다. **급하지 않다** — 재완료 자체가 드물고, 완료 화면은 상태로 이미 보인다
- **고객확인대기에서 처리중으로 되돌릴 수 없다 (2026-08-24 등재 — 미해결)**: D3-2의 전이 표에서 `고객확인대기`의 출구는 `완료` 하나뿐이다 — AC가 "역방향은 재개(완료→처리중)만 허용"이라고 적었고 구현이 그 문면을 따랐다. 그러면 **고객이 "아직 안 됐다"고 했을 때 담당자가 표현할 수단이 없다**: 완료로 올린 뒤 재개해야 하고, 그 순간 거짓 완료일이 한 번 찍히고 감사 이력에도 완료→재개 두 행이 남는다(재개가 완료일을 지우더라도 이력은 남는다). 해소 방향 둘: ①`고객확인대기 → 처리중`을 전이 표에 추가한다(역방향 예외가 둘이 된다) ②고객 확인 실패를 별 상태로 두지 않고 코멘트로 남기게 한다(현행 우회의 명시화). **급하지 않다** — 실사용 전이고 eval 36케이스는 이슈 쓰기를 묻지 않는다
- **이관 알림의 수신자를 구현이 정했다 (2026-08-25 등재 — 확인 필요)**: §8은 `MaintenanceHandedOver`의 구독자를 notification으로, 효과를 "이관 완료 알림"으로만 적었고 **누가 받는지는 정의된 적이 없다**. 구현은 **이관된 사이트의 담당 엔지니어**로 정했다(`NotificationSubscriber.onHandedOver`의 ASSUMPTION 주석) — 이관으로 일이 달라지는 사람이 그들이고, 실행한 PM에게는 보내지 않는다(자기가 방금 한 일이다). 유형은 `PROJECT_COMPLETED`를 재사용했다(사용자 결정 2026-08-25 — 그 열거의 javadoc이 이미 "완료·이관 안내"이고 유형을 늘리면 H1-4 설정 화면의 칸이 함께 는다). 확인이 필요한 갈래: 계약 관리 플래그 보유자도 받아야 하는가(계약이 생겼다는 사실은 그들의 관심사다)
- ~~**`ProjectCompleted`·`ProjectReopened`가 한 번도 발행되지 않는다 (2026-08-25 등재)**~~ — **2026-08-25 같은 날 해소**: `ProjectLifecycleChanged`(Kind 둘)로 발행하고 notification이 구독한다. 완료는 배정 인원에게 안내가 가고, **재개는 그 프로젝트의 미읽음 완료 지연 알림을 회수한다**(F3-3) — 통합 테스트가 재개 → 회수를 관통으로 잠근다. 등재 당시의 지적("능력이고 배선이 아니다")이 정확했다: `withdrawUnread`는 구현·테스트가 다 있었고 부르는 자리만 없었다
