...
/**
 * 조직 트리 탐색 — 가시성 판정이 쓰는 subtree·최상위 부문 계산 (상위 PRD §4-4).
 * 조직 노드는 수십 개 규모라 전체 로드 후 메모리 탐색이 단순·표준이다(재귀 SQL 불요).
 * ASSUMPTION: 규모가 수백 노드를 넘으면 재귀 질의로 하향한다.
 */
public final class OrgTree {
    ...
    public static OrgTree of(List<OrgUnit> units) {
        ...
        for (OrgUnit unit : units) {
            ...
            if (!unit.isRoot()) {
                children.computeIfAbsent(unit.getParentId(), parentId -> new ArrayList<>())
                        .add(unit.getId());
            }
        }
    ...
    /** 노드 자신 + 모든 하위 조직 id — 임의 깊이 (팀 가시성 subtree). */
    ...
    public Long topDivisionIdOf(Long nodeId) {
        ...
        while (visited.add(current)) {
            ...
            OrgUnit parent = unitsById.get(unit.getParentId());

            if (parent == null) {
                throw new IllegalStateException("상위 조직 노드 없음: " + unit.getParentId());
            }
