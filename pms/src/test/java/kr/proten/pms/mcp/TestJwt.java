...
/**
 * 게이트 M0 테스트 토큰 발급기 — HS256 자체 서명 (pms-mcp-mock B2-2 승격분).
 * 클레임 규칙은 구현_노트 §1-2·§1-3과 동일: sub=사용자 id · name · aud=pms ·
 * channel=ai-assistant. 판정에 신뢰하는 클레임은 sub뿐.
...
public final class TestJwt {
    ...
    /** 기본 그룹 4종 대표 (시드 id) — 라벨은 콘솔 인코딩 무관하게 ASCII 선행 */
    record Persona(int id, String label, String name) {
    }

    public static void main(String[] args) {
        // 시크릿 정본 = application.yml — printTestTokens 태스크가 읽어 프로퍼티로 넘긴다
        String secret = System.getProperty("pms.auth.hs256-secret");
        if (secret == null || secret.isBlank()) {
            throw new IllegalStateException("pms.auth.hs256-secret 프로퍼티 필요 — ./gradlew printTestTokens로 실행");
        }
